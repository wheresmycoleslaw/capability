...
for (const pkg of index.packages) {
  ...
  for (const indexed of pkg.capabilities) {
    ...
    if (!remote || typeof remote === "string") throw new Error(`${pkg.name}@${pkg.version} does not publish an inert descriptor for ${indexed.manifest.id}`);
    if (remote.module !== indexed.module) throw new Error(`${pkg.name}@${pkg.version}:${indexed.manifest.id} module differs between npm and registry`);
    if (!equal(remote.manifest, indexed.manifest)) throw new Error(`${pkg.name}@${pkg.version}:${indexed.manifest.id} manifest differs between npm and registry`);
  }
}
...
console.log(JSON.stringify({
  ...
  npmArtifactsVerified: index.packages.filter((pkg) => pkg.source === "npm").length,
  noveltyWarnings
}, null, 2));
