...
function planFingerprint(plan: Omit<CapabilityPlan, "fingerprint">): string {
  return sha256(plan);
}

function normalizeVerification(value: boolean | CapabilityVerification): CapabilityVerification {
...
function assertPlannedEffectsDeclared(planned: readonly CapabilityEffect[], declared: readonly CapabilityEffect[]): void {
  ...
  if (undeclared.length) {
    throw new CapabilityError("INVALID_PLAN", `Plan requested undeclared effects: ${undeclared.join(", ")}`);
  }
}
...
export class CapabilityRuntime {
  ...
  inspect(id: string) {
    return inspectCapability(this.registry.require(id));
  }

  discover(query: DiscoveryQuery | string) {
  ...
  async plan(id: string, input: unknown): Promise<CapabilityPlan> {
    ...
    const details = capability.plan ? await capability.plan(input) : {};
  ...
  async invoke(id: string, input: unknown, options: ExecutionOptions = {}): Promise<CapabilityReceipt> {
    return this.execute(await this.plan(id, input), options);
  }
  ...
  async execute(plan: CapabilityPlan, options: ExecutionOptions = {}): Promise<CapabilityReceipt> {
    ...
    if (capability.manifest.version !== plan.capability.version) {
      throw new CapabilityError("INVALID_PLAN", "Capability version changed after planning");
    }
    ...
    try {
      ...
      if (capability.verify) {
        verification = normalizeVerification(await capability.verify(output, context));
  ...
  async rollback(receiptId: string, options: ExecutionOptions = {}): Promise<CapabilityReceipt> {
    ...
    if (!capability.rollback || capability.manifest.behavior?.reversible !== true) {
    ...
    const plan: CapabilityPlan = deepFreeze({
      ...
      summary: `Rollback ${capability.manifest.name}`,
      createdAt: this.clock().toISOString(),
    ...
    try {
      const rollbackResult = await capability.rollback(context);
      const ended = this.clock();
      const updated: CapabilityReceipt = deepFreeze({
        ...original,
        status: "rolled_back",
        endedAt: ended.toISOString(),
        rollbackResult
  ...
  async getReceipt(receiptId: string): Promise<CapabilityReceipt | undefined> {
    return this.receipts.get(receiptId);
  }

  async listReceipts(): Promise<CapabilityReceipt[]> {
