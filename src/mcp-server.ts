...
const SERVER_INFO = { name: "capability-network", version: "0.6.0" };
...
async function handle(request: JsonRpcRequest) {
  ...
  if (method === "server/discover") {
    return result(id, {
      ...
      capabilities: { tools: {} },
      instructions: "Use capability_search for executable Capability-native abilities, capability_search_world when you need to discover existing external software, capability_inspect before execution, and capability_execute only after reviewing declared effects and required approval.",
      _meta: modernMeta()
    });
  }
  ...
  if (method === "initialize") {
    ...
    return result(id, {
      ...
      instructions: "Capability is a bootstrap MCP server: search the federated ability network or external software catalogs, inspect contracts before code loading, bridge existing tools without rewriting them, and execute Capability-native artifacts through verified isolated acquisition."
    });
  }
